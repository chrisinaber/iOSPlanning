/*: 
 ...
 In Swift, optionals can also be used with value types, e.g. Int.
...
/*: 
 What value do you think that `alternativeMessage` has when it is first defined? What happens if you insert a `print()` statement on line 40 so that you can output the value?
...
/*: 
 ...
 What happens if you go back to your assignment for code and set it to nil? What happens in the print output on (or around) line 90?
...
/*:
 What output do you get from the print if you modify the definition on line 167? Assign an initial value, i.e. = UILabel()
...
//: What if we had a chain of several items that could be optional? We can use Optional Chaining to only access the property lowercaseString if the name contains a value. As an example, compare the previous statement with the following.s
